...
function createEnemyAttack(image, turns){

    did("enemyAttackBox").style.display = "flex"
    voidAnimation("enemyAttackBox", "growFadeIn 0.3s 1")
    did("enemyAttackProgress").style.transition = turns*enemyTurnSpeed+"ms linear"
    did("enemyAttackImg").src = image
    setTimeout(() => {
        did("enemyAttackProgress").style.height = "100%"
    }, 100);
...
function castHoopperoona(){
    ...
    if(enemyTurn===10){
        if (areas[stats.currentArea].heat===1) {enemyTurn=0; return}
        createEnemyAttack(`img/src/buffs/B58.jpg`,3)
    }
