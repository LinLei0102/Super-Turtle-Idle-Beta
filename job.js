...
function createRecipe() {
   ...
   if (!did(r+"recipe")) {
    ...
    recipediv.innerHTML = '<span id="'+r+'recipeLevel">'+recipes[r].level+'</span><img id="'+r+'recipeImage" src="img/src/items/'+recipes[r].item+'.jpg"><strong id="'+r+'recipeName">?????</strong><span style="margin-left: auto; background: #364D68; color: #7ACCDE; transition:0s !important;" id="'+r+'craftQueue"><img id="'+r+'craftIcon" src="img/src/icons/craftOne.png">200</span>';
   ...
   if (did(r+"recipe")) {
    ...
    if (recipes[r].unlocked === true && did(r+"recipe").style.display === "none" )  {did(r+"recipe").style.display = "flex"}
     ...
     did(r+'craftQueue').style.background = "transparent";



     if (recipes[r].level <= (jobs[rpgPlayer.currentJob].level)) {
        did(r+"recipeLevel").style.background = "#FF993B"; 
        did(r+'craftQueue').style.color = "#FF993B";
        did(r+'craftQueue').innerHTML = beautify(recipes[r].exp*1.3) +" XP";

     }
       
     if (recipes[r].level <= (jobs[rpgPlayer.currentJob].level - 5)) {
        did(r+"recipeLevel").style.background = "#6BAD51";  
        did(r+'craftQueue').style.color = "#6BAD51";
        did(r+'craftQueue').innerHTML = beautify(recipes[r].exp) +" XP";
     ...
     //did(r+'craftQueue').style.display = "flex";
     ...
     if (recipes[r].crafting==='all' || recipes[r].crafting==='once') {
        did(r+'craftQueue').style.background = "#364D68";
...
function updateRecipeButton(r) {
    if(recipes[r].level <= (jobs[rpgPlayer.currentJob].level)){ //clickable if the player has enough level
       ...
       if (recipes[r].level <= (jobs[rpgPlayer.currentJob].level)) {
        ...
        did('recipeExp').innerHTML = "&nbsp"+beautify(recipes[r].exp*1.3) +" XP";

     }
       
     if (recipes[r].level <= (jobs[rpgPlayer.currentJob].level - 5)) {
        did('recipeExp').style.color = "#6BAD51";
        did('recipeExp').innerHTML = "&nbsp"+beautify(recipes[r].exp) +" XP";
     }
     ...
     if(recipes[r].level <= (jobs[rpgPlayer.currentJob].level - 10)) {
        ...
        did('recipeExp').innerHTML = "&nbsp0 XP";
    ...
    if (count==="all") itemQueueValue = 1;
     ...
     if (canCraft && (recipes[currentRecipe].crafting === "false" || (recipes[currentRecipe].craftingQueue>=0 && count !== "all")) && recipes[currentRecipe].crafting !== "all") {
         ...
         if (count === "once"){
            ...
            did(currentRecipe+'craftQueue').style.animation = '';
         void did(currentRecipe+'craftQueue').offsetWidth;
         did(currentRecipe+'craftQueue').style.animation = 'gelatine 0.3s 1';
...
function craftingProgress(){
    ...
    let expmult = 1
    if (recipes[r].level >= (jobs[rpgPlayer.currentJob].level -5)) expmult = 1.3
    ...
    if (r.startsWith("S")) jobs.blacksmith.exp += recipes[r].exp*expmult;
    if (r.startsWith("C")) jobs.cooking.exp += recipes[r].exp*expmult;
    if (r.startsWith("A")) jobs.alchemy.exp += recipes[r].exp*expmult;
    if (r.startsWith("E")) jobs.engineering.exp += recipes[r].exp*expmult;
